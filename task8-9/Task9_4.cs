        ...
        while (true)
        {
            ...
            switch (syote)
            {
                ...
                case "2":
                    ...
                    // Выводим пронумерованный список фильмов для удаления
                    for (int i = 0; i < leffat.Count; i++)
                    {
                        Console.WriteLine($"{i + 1}) {leffat[i].Nimi} ({leffat[i].Vuosi}), {leffat[i].Kesto} minuuttia.  ");
                    }
                    
                    // Запрашиваем номер фильма для удаления
                    Console.Write("Syöte: ");
                    int valinta = int.Parse(Console.ReadLine());
                    
                    // Удаляем элемент по его индексу в списке (порядковый номер минус 1)
